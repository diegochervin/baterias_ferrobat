...
const ListaBaterias = ({ baterias }) => {
  return (
    <div className="container mt-4">
      {baterias.length === 0 ? (
        <p className="text-center">No se encontraron baterías que coincidan con los filtros.</p>
      ) : (
    ...
    <Col key={bateria.id} style={{ flex: '0 0 auto' }}>
      ...
      >
        ...
        <Card.Body className="d-flex flex-column" style={{ flexGrow: 1 }}>
          <Card.Title>{bateria.marca} {bateria.modelo}</Card.Title>
          <Card.Text className="mb-4">
            <strong>Precio:</strong> ${bateria.precio} <br />
            <strong>Stock:</strong> {bateria.stock}
          </Card.Text>
          <Button
            variant="primary"
